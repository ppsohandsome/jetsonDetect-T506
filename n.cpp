...
void getframe(std::string camid,std::string rtsp_url,int width,int height){
	...
	std::cout<<"latency"<<options.latency<<std::endl;
	std::cout<<"device:"<<options.codecType<<std::endl;
...
options.latency = 100;
	...
	while(true){
		...
		std::string currentTime3 = getCurrentTimeStr();		
		if(!input->Capture(&data, 100, &status)){
		...
		std::string currentTime4 = getCurrentTimeStr();		
		std::cout<<"aaaaaaaaaaaaaaaaa:"<<currentTime3<<" "<<currentTime4<<std::endl;
		...
		if(saveindex++%5==0){
			...
			std::cout<<"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"<<std::endl;
			std::string currentTime = getCurrentTimeStr();		
			cv::imwrite("/home/nvidia/project/jetsonDetect/build/saveimage/"+camid+"_"+currentTime+"_frame.jpg",frame);
			std::string currentTime2 = getCurrentTimeStr();		
			std::cout<<"currrrrrrrrrrrrrrrrrrr:"<<currentTime<<" "<<currentTime2<<std::endl;
/*			
			mtx.lock();
...
int main()
{
	...
	videoSource* input = videoSource::Create(config.rtspList[0].rtspPath.c_str(), options);
	int status = 0;
	int saveindex = 0;
	uchar3 * data;
	...
	while(true){
		...
		if(saveindex++%5==0){
			std::cout<<"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"<<std::endl;
			cv::Mat frame = uchar3ToCvMat(data,1920,1080);
			// cv::Mat frame = uchar3ToCvMat(data,1024,768);
			FrameSet frameset;
			frameset.camid = "001";
			frameset.frame = frame.clone();
			std::cout<<"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"<<std::endl;
			std::string currentTime = getCurrentTimeStr();		
			cv::imwrite("/home/nvidia/project/jetsonDetect/build/saveimage/05_"+currentTime+"_frame.jpg",frame);
			std::string currentTime2 = getCurrentTimeStr();		
			std::cout<<"currrrrrrrrrrrrrrrrrrr:"<<currentTime<<" "<<currentTime2<<std::endl;
/*			
			mtx.lock();
